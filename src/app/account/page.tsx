...
async function AccountPageContent() {
  ...
  const profileCompletion = {
    ...
    hasBio: !!profile?.bio,
    hasLinks: true, // Assuming true for now
    isDiscordLinked: !!discordIdentity,
    is2FAEnabled: false, // Assuming false for now
  }
  
  const completionSteps = Object.values(profileCompletion);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
            <div>
              <h2 className="text-2xl font-semibold mb-4">Account Statistics</h2>
              <Card className="bg-card/50 border-white/10">
                  <CardContent className="p-6">
                      <p className="font-medium">Profile Completion</p>
                      <div className="flex items-center gap-4 mt-2">
                          <Progress value={completionPercentage} className="h-2" />
                          <span className="text-sm font-medium text-muted-foreground">{completionPercentage}% completed</span>
                      </div>
                      <div className="mt-4 p-4 rounded-md bg-yellow-500/10 border border-yellow-500/20 flex items-center gap-3">
                          <AlertTriangle className="h-5 w-5 text-yellow-400" />
                          <div>
                              <p className="font-semibold text-yellow-400">Your profile isn't complete yet!</p>
                              <p className="text-sm text-yellow-400/70">Complete your profile to make it more discoverable and appealing.</p>
                          </div>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6 text-sm">
                          <Button variant="outline" className={`justify-start border-dashed ${profileCompletion.hasAvatar && 'border-solid border-green-500/30 bg-green-500/10'}`}>
                             {profileCompletion.hasAvatar ? <CheckCircle className="h-4 w-4 mr-2 text-green-400" /> : <UserCog className="h-4 w-4 mr-2" />} Upload An Avatar
                          </Button>
                           <Button variant="outline" className={`justify-start border-dashed ${profileCompletion.hasBio && 'border-solid border-green-500/30 bg-green-500/10'}`}>
                             {profileCompletion.hasBio ? <CheckCircle className="h-4 w-4 mr-2 text-green-400" /> : <PenSquare className="h-4 w-4 mr-2" />} Add A Description
                          </Button>
                          <Button variant="outline" className={`justify-start border-dashed ${profileCompletion.hasLinks && 'border-solid border-green-500/30 bg-green-500/10'}`}>
                              {profileCompletion.hasLinks ? <CheckCircle className="h-4 w-4 mr-2 text-green-400" /> : <LinkIcon className="h-4 w-4 mr-2" />} Add Socials
                          </Button>
                           <Button variant="outline" className={`justify-start border-dashed ${profileCompletion.isDiscordLinked && 'border-solid border-green-500/30 bg-green-500/10'}`}>
                            {profileCompletion.isDiscordLinked ? <CheckCircle className="h-4 w-4 mr-2 text-green-400" /> : <LinkIcon className="h-4 w-4 mr-2" />} Link Discord
                          </Button>
                          <Button variant="outline" className={`justify-start border-dashed ${profileCompletion.is2FAEnabled && 'border-solid border-green-500/30 bg-green-500/10'}`}>
                             {profileCompletion.is2FAEnabled ? <CheckCircle className="h-4 w-4 mr-2 text-green-400" /> : <Shield className="h-4 w-4 mr-2" />} Enable 2FA
                          </Button>
                      </div>
                  </CardContent>
              </Card>
            ...
            <div className="max-w-full mx-auto bg-card/50 rounded-xl border border-white/10">
              {user && profile && <AccountForm user={user} profile={profile} />}
            </div>
                     ...
                     {discordIdentity ? (
                        <div className="flex items-center justify-between p-3 rounded-md bg-blue-500/10 border border-blue-500/20">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-semibold text-white">Discord Connected</p>
                                    <p className="text-xs text-muted-foreground">{discordIdentity.identity_data?.user_name}</p>
                                </div>
                            </div>
...
export default async function AccountPage() {
  ...
  return (
    <div className="flex-grow p-6 md:p-8 lg:p-12 space-y-8">
