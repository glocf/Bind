...
export default function LoginPage() {
    ...
    useEffect(() => {
        ...
        if (urlMessage) {
            setMessage({ type: 'info', text: urlMessage })
        }
    }, [searchParams])
        ...
        <div className="flex flex-col min-h-screen bg-background">
            <div className="flex-grow flex items-center justify-center">
                <Card className="w-full max-w-sm mx-auto">
                    ...
                    <CardContent>
                        {message && (
                            <Alert variant={message.type === 'info' ? 'default' : 'destructive'} className="mb-4">
                                <Terminal className="h-4 w-4" />
                                <AlertTitle>{message.type === 'info' ? 'Info' : 'Error'}</AlertTitle>
                                <AlertDescription>
                                    {message.text}
                                </AlertDescription>
                            </Alert>
